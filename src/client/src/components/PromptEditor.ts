...
export class PromptEditor extends LitElement {
  ...
  protected override willUpdate(changed: PropertyValues<this>) {
    ...
    this.attachments = memory?.attachments ?? (currentKey !== undefined ? [...loadStagedAttachments(currentKey)] : []);
  ...
  private applyFailedDelivery(originMachineId: string, originSessionId: string | undefined, originKey: string | undefined, warning: string): void {
    if (this.isCurrentComposer(originMachineId, originSessionId)) {
      if (originKey !== undefined) {
        ...
        saveStagedAttachments(originKey, this.attachments);
    ...
    const memory = this.composerMemory.get(originKey) ?? {
      attachments: [...loadStagedAttachments(originKey)],
  ...
  private applyAcceptedDelivery(
    ...
    if (!this.isCurrentComposer(originMachineId, originSessionId)) {
      if (originKey !== undefined) {
        ...
        const memory = this.composerMemory.get(originKey);
        const remaining = (memory?.attachments ?? loadStagedAttachments(originKey)).filter((attachment) => !submittedAttachmentIds.has(attachment.id));
        saveStagedAttachments(originKey, remaining);
        this.composerMemory.set(originKey, {
